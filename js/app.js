(() => {
    ...
    formSubmit();
    function initBurger(burgerButton, burgerMenu, closingOut, escapeClose) {
        const BUTTON = document.querySelector(`.${burgerButton}`);
        const MENU = document.querySelector(`.${burgerMenu}`);
        document.addEventListener("click", burgerClicks);
        if (escapeClose) document.addEventListener("keydown", burgerKey);
        function burgerOpen() {
        ...
        function takeBurgerState() {
            return document.documentElement.classList.contains("menu-active") ? true : false;
        }
        function burgerClicks(event) {
            if (takeBurgerState()) {
                if (event.target != MENU && !MENU.contains(event.target) && closingOut || event.target == BUTTON) burgerClose();
            } else if (event.target == BUTTON) burgerOpen();
            const goTarget = event.target.closest("[data-go]");
            if (goTarget) document.documentElement.classList.remove("menu-active");
        }
        function burgerKey(event) {
            if (event.code.toLowerCase() === "escape") burgerClose();
        }
    }
    initBurger(`button-menu`, `menu`, true, true, `menu-active`);
    function handleScroll() {
        const header = document.querySelector(".header");
        const scrollPosition = window.scrollY;
        if (scrollPosition > 0) header.classList.add("active-scroll"); else header.classList.remove("active-scroll");
    }
    window.addEventListener("scroll", handleScroll);
    handleScroll();
    function slideSpollers() {
    ...
    function initNav(dataNav, dataSections, isSmooth, header) {
        ...
        dataNav = dataNav.toLowerCase();
        const HEADER = header ? document.querySelector(`.${header}`) : false;
        BUTTONS.forEach((x => {
            x.onclick = event => {
                if (x.getAttribute("href") === "#") event.preventDefault();
                window.scrollTo({
                    top: (() => {
                        const ELEMENT = document.querySelector(`[data-${dataSections}="${x.dataset[dataNav]}"]`);
                        if (HEADER) return ELEMENT.getBoundingClientRect().top - HEADER.offsetHeight; else return ELEMENT.getBoundingClientRect().top;
                    })(),
                    left: 0,
